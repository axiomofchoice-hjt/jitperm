...
jit_fn<int32_t> gen_kernel<int32_t, ISA::SSEPack>(std::span<const size_t> permutation) {
    ...
    return jit::gen<kernel_fn<int32_t>>([=](asmjit::x86::Assembler& as) {
        ...
        for (size_t i : std::views::iota(size_t{0}, target_size / vector_size * vector_size) | std::views::stride(4)) {
            ...
            as.movd(xmm, x86::dword_ptr(input_ptr, permutation[i] * sizeof(int32_t)));
            as.pinsrd(xmm, x86::dword_ptr(input_ptr, permutation[i + 1] * sizeof(int32_t)), 1);
            as.pinsrd(xmm, x86::dword_ptr(input_ptr, permutation[i + 2] * sizeof(int32_t)), 2);
            as.pinsrd(xmm, x86::dword_ptr(input_ptr, permutation[i + 3] * sizeof(int32_t)), 3);
            as.movups(x86::xmmword_ptr(output_ptr, i * sizeof(int32_t)), xmm);
        }
